...
# Run apt update before installing packages --- onboard error (python "firmware-qcom-soc firmware-linux")
apt update
...
apt install -y qcom-phone-utils firmware-iwlwifi firmware-qcom-media || true
#########
...
[begin] (Pro 1X Test Menu)
    [exec] (Keyboard) {xvkbd}
    [exec] (SCREEN 2 ) {xrandr --output DSI-1 --scale .2x.2}
    [exec] (SCREEN 3) {xrandr --output DSI-1 --scale .3x.3}
    [exec] (SCREEN 4) {xrandr --output DSI-1 --scale .4x.4}
    [exec] (SCREEN 5) {xrandr --output DSI-1 --scale .5x.5}
    [exec] (Terminal) {xterm}
...
# enable menu and fix toolbar
sudo sed -i '/^OnDesktop Mouse3 :RootMenu/{s/^OnDesktop Mouse3 :RootMenu/OnDesktop Double Mouse1 :RootMenu/; a\
220 :RootMenu
}' $ROOTFS_DIR/home/user/.fluxbox/keys

echo -e "\nsession.screen0.toolbar.placement: TopCenter" | sudo tee -a $ROOTFS_DIR/home/user/.fluxbox/init > /dev/null
